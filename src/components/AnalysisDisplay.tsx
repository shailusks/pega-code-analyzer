    ...
    <div className="mt-8">
      ...
      {isPegaAnalysisError(currentAnalysis) ? (
        ...
        <div className="space-y-8">
          ...
          <div>
            <h3 className="text-2xl font-semibold text-gray-800 mb-6 mt-10">Detailed Feedback & Suggestions for <span className="italic text-sky-700">{activeFileAnalysis.fileName}</span></h3>
            {currentAnalysis.feedbackItems.length > 0 ? (
              <div className="space-y-4">
                {currentAnalysis.feedbackItems.map((item) => (
                  <FeedbackItemCard 
                    key={item.id} 
                    item={item}
