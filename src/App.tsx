...
import { 
    ...
    LlmProvider, LLM_PROVIDER_NAMES, AnalysisFunction, ChatFunction 
} from './types';
...
const App: React.FC = () => {
  ...
  // Azure OpenAI specific endpoint state
  ...
  const [effectiveAzureEndpoint, setEffectiveAzureEndpoint] = useState<string>(''); // Only for Azure OpenAI
  ...
  const handleSetUserApiKeyAndEndpoint = () => {
    ...
    if (selectedLlmProvider === LlmProvider.AZURE_OPENAI) {
        ...
        if(endpointToSet) {
            ...
            success = success && true; // both must be set or come from env for full success state
        } else {
  ...
  const commonAnalyzeAction = async (analysisFnExecutor: (analyzeSvc: AnalysisFunction, model: string, instruction: string, endpoint?: string) => Promise<PegaFileAnalysis[] | PegaFileAnalysis>) => {
    ...
    try {
      const result = await analysisFnExecutor(analyze, analysisModel, analysisSystemInstruction, effectiveAzureEndpoint);
  ...
  }, [xmlInput, effectiveApiKey, selectedLlmProvider, effectiveAzureEndpoint, envApiKeys]);
  ...
  }, [zipFile, effectiveApiKey, selectedLlmProvider, effectiveAzureEndpoint, envApiKeys]);
  ...
  }, [analysisData, effectiveApiKey, selectedLlmProvider, effectiveAzureEndpoint, envApiKeys]);
